          ...
          <div className="border-b border-[#dce5ee] lg:border-b-0 lg:border-r">
            ...
            <div className="p-5 sm:p-6">
              <div className="flex items-start justify-between gap-4">
                <div><p className="text-xs font-bold uppercase text-[#718096]">Recognized meal</p><h3 className="mt-2 text-xl font-semibold text-[#0b1f33]">Salmon rice bowl</h3><p className="mt-1 text-sm text-[#526477]">12:35 PM - Fictional demo</p></div>
                ...
                {mealEstimates.map((item, index) => (
                  <div key={item.food} className="grid grid-cols-[1fr_auto] gap-4 py-4">
                    ...
                    <div className="text-right">
                      {editing ? (
                        <label className="block text-xs font-semibold text-[#526477]"><span className="sr-only">Estimated carbohydrates for {item.food}</span><input type="number" min="0" max="200" value={carbohydrates[index]} onChange={(event) => { const next = [...carbohydrates]; next[index] = Number(event.target.value); setCarbohydrates(next); }} className="h-9 w-20 rounded-lg border border-[#9eb4c9] px-2 text-right text-sm text-[#0b1f33]" /><span className="ml-1">g</span></label>
